; test_stack_debug.asm - Debug the crash with proper stack
bits 64
default rel
...
.clr:
    ...
    mov qword [rdi + 0x6FFF00], 1
    mov qword [rdi + 0x6FFF08], 0x6FE000
    mov qword [rdi + 0x6FFF10], 0
    mov qword [rdi + 0x6FFF18], 0
    mov qword [rdi + 0x6FFF20], 0
    mov qword [rdi + 0x6FFF28], 0

    lea rax, [rv_regs]
    mov qword [rax + 2*8], 0x6FFF00
    mov qword [rdi + 0xF000], 0x200000

    ; Run in chunks of 100000 blocks, print progress
...
.have_pc:
    ...
    ; Check if we hit 10M blocks or PC went weird
    cmp qword [iter_count], 10000000
    jge .done
