; test_envp.asm - Check what a0/a2 point to at the loop
bits 64
default rel
...
section .data
    ...
    msg_mem         db " -> memory: ", 0
    msg_nl          db 13, 10, 0
    msg_envp        db "Setting up envp...", 13, 10, 0
...
.clr:
    ...
    ; Set argc/argv - but also need envp!
    ...
    mov qword [rdi + 0x6FF000], 0x6FE000  ; argv[0]
    mov qword [rdi + 0x6FF008], 0         ; argv[1] = NULL

    ; envp array at 0x6FF010 - needs to have NULL terminator!
    mov qword [rdi + 0x6FF010], 0         ; envp[0] = NULL (empty env)

    lea rax, [rv_regs]
    mov qword [rax + 10*8], 1             ; a0 = argc = 1
    mov qword [rax + 11*8], 0x6FF000      ; a1 = argv
    mov qword [rax + 12*8], 0x6FF010      ; a2 = envp (!!!)

    mov qword [rdi + 0xF000], 0x200000    ; brk

    ; Print that we set up envp
