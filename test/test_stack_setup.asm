; test_stack_setup.asm - Proper Linux stack setup for RISC-V
; Stack layout at _start:
;   sp+0:  argc
;   sp+8:  argv[0]
;   sp+16: argv[1] ... (NULL terminated)
;   sp+N:  NULL
;   sp+N+8: envp[0], envp[1], ... (NULL terminated)
;   sp+M:  NULL
;   sp+M+8: auxv entries...
...
section .data
    ...
    msg_loading     db "Loading Doom with proper stack...", 13, 10, 0
...
.clr_regs:
    ...
    ; Stack at 0x700000, growing down
    ; Put stack data below 0x700000
    ;
    ; Layout (addresses relative to guest_memory):
    ; 0x6FFF00: argc = 1
    ; 0x6FFF08: argv[0] = 0x6FE000 (pointer to "doom")
    ; 0x6FFF10: argv[1] = NULL
    ; 0x6FFF18: envp[0] = NULL  <-- THIS IS THE KEY FIX!
    ; 0x6FFF20: auxv[0] = AT_NULL (0)
    ; 0x6FFF28: auxv[1] = 0

    mov qword [rdi + 0x6FFF00], 1          ; argc = 1
    mov qword [rdi + 0x6FFF08], 0x6FE000   ; argv[0] = "doom"
    mov qword [rdi + 0x6FFF10], 0          ; argv[1] = NULL (end of argv)
    mov qword [rdi + 0x6FFF18], 0          ; envp[0] = NULL (end of envp)
    mov qword [rdi + 0x6FFF20], 0          ; auxv type = AT_NULL
    mov qword [rdi + 0x6FFF28], 0          ; auxv value = 0

    ; Set sp to point to argc on stack
    lea rax, [rv_regs]
    mov qword [rax + 2*8], 0x6FFF00        ; sp points to argc
