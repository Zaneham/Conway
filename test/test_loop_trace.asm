; test_loop_trace.asm - Trace x12 changes during the spin loop
bits 64
default rel
...
.clr:
    ...
    ; Run until we hit the spin loop (around block 25)
    ...
    ; Now trace x12 for the next iterations
    mov qword [iter_count], 0
    mov qword [last_x12], 0
...
.trace_loop:
    ...
    add qword [iter_count], 1000

    ; Check if x12 changed
    lea rax, [rv_regs]
    mov rcx, [rax + 12*8]
    cmp rcx, [last_x12]
    je .skip_print
...
.skip_print:
    ; Stop after 1M iterations or if PC changed from loop
    cmp qword [iter_count], 1000000
    jge .done
